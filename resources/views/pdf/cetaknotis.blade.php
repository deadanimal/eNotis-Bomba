...
<body>
    ...
    <br>
    @endforeach
    <br>SILA AMBIL PERHATIAN bahawa Ketua Pengarah Perkhidmatan Bomba +, setelah berpuas hati tentang adanya di {{ $premis->nama_premis}}, {{$premis->alamat}} suatu bahaya kebakaran, iaitu {{ $notis->jenis_ppk }} yang {{ $notis->kesalahan }} di bahagian {{$notis->lokasi}} {{$notis->aras}} premis, dibawah {{$notis->seksyen}} yang boleh menyebab ancaman kepada keselamatan nyawa dan harta benda jika berlaku kebakaran</p>
    ...
    <p>Bertarikh pada date('d-M-Y', strtotime({{$notis->tarikh_pemeriksaan}}));
        {{$notis->tarikh_pemeriksaan}}->format('d/m/Y')
